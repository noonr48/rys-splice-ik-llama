...
struct llama_context * llama_init_from_model(
    ...
    cparams.rys_splice_fastpath = params.rys_splice_fastpath && model->supports_rys_splice_fastpath();
    cparams.min_experts      = params.min_experts;
    cparams.thresh_experts   = params.thresh_experts;
    cparams.cuda_params      = params.cuda_params;
    cparams.mtp              = params.mtp;

    if (params.rys_splice_fastpath && !cparams.rys_splice_fastpath) {
        LLAMA_LOG_WARN("%s: --rys-splice-fastpath requested, but model is not a supported Qwen3.5 IQ4_NL target\n", __func__);
    }

    const bool split_mode_graph_requested = model->split_mode == LLAMA_SPLIT_MODE_GRAPH;
    const bool split_mode_graph_enabled =
            split_mode_graph_requested &&
            (!model->has_tensor_overrides() || cparams.split_mode_graph_scheduling);

    if (cparams.rys_splice_fastpath && split_mode_graph_requested && !split_mode_graph_enabled) {
        LLAMA_LOG_WARN("%s: RYS splice fast path requested while split mode 'graph' is blocked by tensor overrides; keeping existing scheduler behavior\n", __func__);
    }
    ...
    LLAMA_LOG_INFO("%s: rys_splice_fp = %d\n",     __func__, cparams.rys_splice_fastpath);

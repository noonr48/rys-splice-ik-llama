...
| `--rys-splice-fastpath` | Enable the specialized RYS splice IQ4_NL fast path when supported | 0 | Fork-specific opt-in for the narrow RYS splice Qwen 3.5 IQ4_NL path. It does not override split mode `graph`. Right now it enables run-time repack at model load, forces F32 KV cache, and only activates the execution-side fastpath toggle when the loaded model is a supported Qwen 3.5 IQ4_NL target. |
